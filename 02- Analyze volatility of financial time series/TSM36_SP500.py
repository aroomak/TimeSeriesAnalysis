...
def graph_export_d(data_in,data_pf,xi,export_path):
    x_value=data_in.x
    ...
    ## Graph Export
    # fig1.savefig(export_path+'SP500_Graph1_x_and_ht_hat.png',dpi=300,bbox_inches='tight')
    # fig2.savefig(export_path+'SP500_Graph2_Ht.png',dpi=300,bbox_inches='tight')
    # fig3.savefig(export_path+'SP500_Graph3_PF_Ht.png',dpi=300,bbox_inches='tight')
...
def dataplot1():
    ...
    fig.set_figwidth(20)
    plt.show() 
    # fig.savefig(plot_path+'c1-dataplot.png', dpi=300)
...
def dataplot2():
    ...
    sns.scatterplot(data=data, x="t", y="y",  legend=False, sizes=(20, 2000), ax=axs[0])
    sns.distplot(a=data.y, hist=True, kde=False, rug=False, color="red", ax=axs[1]) 
    sns.distplot(data["y"],
             kde=True,
             kde_kws={"color": "g", "alpha": 0.3, "linewidth": 5, "shade": True}, 
             ax=axs[2])
    plt.show()
    # fig.savefig(plot_path+'c2-dataplot.png', dpi=300)

def dataplot3():
    fig, ax = plt.subplots()
    plt.style.use('seaborn-darkgrid')
    ax.plot(data.x, color='b')
    fig.set_figheight(5)
    fig.set_figwidth(20)
    plt.show() 
    # fig.savefig(plot_path+'c3-dataplot.png', dpi=300)
...
omega=-0.6470781523624332        
phi=0.9343008802713368          
sig2_eps=np.square(np.pi)/2      ## variance of log-chi square(1)
sig2_eta=0.13401119194050845
xi= omega/(1-phi)
